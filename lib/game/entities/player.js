...
.defines(function(){
	EntityPlayer = ig.Entity.extend({
		...
		friction: 0.3,				// this may be useless for box 2D entities
		accelGround: 1.25,			// this may be useless for box 2D entities
		accelAir: 0.1,				// this may be useless for box 2D entities
		jump: 25,					// jumping originally set to 500 in non-physics engine
		health: 20,
		weapon: 2,
		totalWeapons: 3,
		...
		categoryBits: 0x0001,	// collision type friendly
		...
		init: function( x, y, settings) {
			...
			this.makeInvincible();
			// reset the player pointer for screen following
		...
		update: function() {
			...
			//jump
			if( ig.input.pressed('jump') && this.jumpCounter >= 0) {
				var jumpScale = 1;
				this.jumpCounter -= 1;
				if( ig.input.state('duck') ) {jumpScale=2}
				this.body.ApplyImpulse( new Box2D.Common.Math.b2Vec2(0,-this.jump*jumpScale), this.body.GetPosition() );
				this.jumpSFX.play();
			}
			if( (this.body.m_linearVelocity.y < 2) && (this.body.m_linearVelocity.y > -2) ) { this.jumpCounter = 0 }
			...
			if( ig.input.pressed('punch') ) {
				ig.game.spawnEntity( "EntityDragonPunch", this.pos.x, this.pos.y, {flip:this.flip}); //xInitVel
				this.body.ApplyImpulse( new Box2D.Common.Math.b2Vec2(this.flip ? 2 : -2 ,0), this.body.GetPosition());
		...
		kill: function () {
			...
			// flag to disable screen following during respawn
			ig.game.respawnFlag = 0;

			this.deathSFX.play();

			// peform explosion animation & respawn the player after death
	...
	EntityRocketSmoke = ig.Entity.extend({
		...
		callBack: null,
		isFixedRotation: true,
		categoryBits: 0x0000,	// collision type friendly
		maskBits: 0x0000,		// collides with enemy
	...
	EntityDragonPunch = ig.Entity.extend({
		...
		init: function(x, y, settings){
			this.parent(x + (settings.flip ? -15 : 15), y, settings );
			//this.body.SetLinearVelocity( (settings.flip ? -10: 10), 0 );
			this.body.ApplyImpulse( new Box2D.Common.Math.b2Vec2(this.flip ? -this.maxVel.x : this.maxVel.x, 0), this.body.GetPosition() );											
